		...
		if f, err := s.readFrame(buffer); err == nil {
			...
			case cmdNOP:
				if s.EnableStreamBuffer {
					...
					s.writeFrameNRet(newFrame(cmdNOP, f.sid))
				}
			case cmdSYN:
				s.streamLock.Lock()
...
func (s *Session) keepalive() {
	min := s.config.KeepAliveInterval
	delta := (s.config.KeepAliveIntervalMax - s.config.KeepAliveInterval).Nanoseconds() + 1
	next := min + time.Duration(rand.Int63n(delta))
	t := time.NewTimer(next)

	s.rttTest = time.Now()
	s.writeFrame(newFrame(cmdNOP, atomic.AddUint32(&s.rttSn, uint32(1))))
	ckTimeout := time.NewTimer(next + s.config.KeepAliveTimeout) // start timeout check
	...
	for {
		...
		case <-t.C:
			...
			t.Reset(min + time.Duration(rand.Int63n(delta))) // setup next ping
		case <-s.die:
			// clear all timer
			stopT()
			stopCk()
